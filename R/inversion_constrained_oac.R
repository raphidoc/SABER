#' inversion_constrained_oac

inversion_constrained_oac <- function() {
  # OAC variable constrained inversion --------------------------------------

  if (constrained_oac == TRUE) {
    #initial <- abs(initial)
    if (obj_fn == "log-LL") {
      ...
      NLL_constr = function(pars, data) {
        ...
        if (sa_model == "am03") {
          Gpred = Saber_forward_fast(
            ...
            chl = constrain_oac_value[1],
            a_dg = constrain_oac_value[2],
            bbp.550 = constrain_oac_value[3],
