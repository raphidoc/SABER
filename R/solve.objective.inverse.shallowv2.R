...
solve.objective.inverse.shallow.constrained <- function(constrained,obj.fn,initial,obsdata,sa.model, 
                                            method.opt,lower.b, upper.b, batch, pop.sd){
  myFun <- function(x) {
    NA
  }
  if (constrained == TRUE) {
    ...
    if (obj.fn == "log-LL") {
      ...
      NLL = function(pars, data) {
        ...
        # Values predicted by the forward model for single RUN
        if (sa.model == "am03") {
          Gpred = Saber_forward(chl = Fit.input$chl, acdom440 = Fit.input$acdom.440,
          ...
          Gpred = Lee_forward(chl = Fit.input$chl, acdom440 = Fit.input$acdom.440,
                              anap440 = Fit.input$anap.440, bbp.550 = Fit.input$bbp.550,
