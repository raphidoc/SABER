...
Saber_forward_paramteric_conc <-  function(chl=4.96, acdom440=NULL, anap440=NULL, 
  ...
  if (is.null(base.CDOM) & is.null(base.NAP)) {
    ...
    print("absorption of CDOM and NAP will be combined using QAAv5")
    base_CDM = a_dg
  ...
  if (dg_composite == TRUE) {
    if (use_spectral_shape_dg == TRUE) {
      ...
      print(paste0("CDOM+NAP absorption calculated using spectral shape and dynamic slope from ", min(lambda), " nm to ", max(lambda), " nm."))
      ...
      print(paste0("CDOM+NAP absorption calculated using absolute values and dynamic slope from ", min(lambda), " nm to ", max(lambda), " nm."))
  ...
  if (sicf == TRUE & dg_composite == FALSE) {
    ...
    print(paste0("Absorption of CDOM and NAP is user defined inputs"))
  ...
  if (sicf == TRUE & dg_composite == TRUE) {
    ...
    rrs_sicf = Rrs_Fluorescence(c_chl = C_ph,  
                                ...
                                abs_cdom_443 = base.CDOM,
                                abs_nap_443 = base.NAP, phi_f = q_phi)
    print(paste0("Absorption of CDOM and NAP iscalculated from QAA"))
    print(paste0("Subsurface (0^-) Rrs equivalent to SICF calculated with quantum yield of ", q_phi))
    Rrs_below = Rrs_below + rrs_sicf
  }
