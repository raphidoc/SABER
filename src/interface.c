...
SEXP c_saber_version(void)
{
  return Rf_mkString(saber_version());
}
...
SEXP c_load_pure_water(SEXP s_wl, SEXP s_a_w) {
  ...
  int status = load_pure_water(wl, a_w, n);

  SEXP out = PROTECT(Rf_ScalarInteger(status));
  UNPROTECT(1);
  return out;
}
...
SEXP c_load_a0_a1(SEXP s_wl, SEXP s_a0, SEXP s_a1) {
  ...
  int status = load_a0_a1(wl, a0, a1, n);
                   ...
                   SEXP s_r_rs_b_matrix)
{
  ...
  int status = load_r_rs_b(wl, class_ptrs, matrix,
                           (size_t)n_wl, (size_t)n_class);
...
SEXP c_retrieve_r_rs_b_am03(
  ...
  // ---- Observed Rrs ----
  const double *r_rs_obs = REAL(s_r_rs_obs);
  ...
  // ---- Output vector ----
  SEXP result = PROTECT(allocVector(REALSXP, n));
  double *r_rs_b = REAL(result);

  retrieve_r_rs_b_am03(
    wl, a, bb, r_rs_obs, (size_t)n,
    water_type, theta_sun, theta_view,
    h, r_rs_b
  );
...
/* Debug function to get the data cache */

SEXP c_build_cache(SEXP s_wavelength) {
  int n = LENGTH(s_wavelength);
  const double *wl = REAL(s_wavelength);

  int status = build_cache(wl, n);
...
SEXP c_saber_reset_tables(void)
{
  saber_reset_tables();      /* your massive free() routine   */
return R_NilValue;         /* nothing to return             */
}

SEXP c_get_a_w() {
  const double *p = get_a_w();
  size_t n        = get_n_wl();

  if (p == NULL || n == 0)
    Rf_error("a_w cache is empty – build it before calling c_get_a_w().");

  SEXP out = PROTECT(Rf_allocVector(REALSXP, (R_xlen_t)n));
  memcpy(REAL(out), p, n * sizeof(double));
