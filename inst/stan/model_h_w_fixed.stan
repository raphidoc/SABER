...
transformed parameters {
  ...
  {
    vector[n_wl] sd1 = to_vector(r_b_sd_lib[, bottom_class_ids[1]]);
    vector[n_wl] sd2 = to_vector(r_b_sd_lib[, bottom_class_ids[2]]);
    vector[n_wl] sd3 = to_vector(r_b_sd_lib[, bottom_class_ids[3]]);
    r_b_sd_mix = sqrt(
        square(r_b_mix[1]) * square(sd1)
      + square(r_b_mix[2]) * square(sd2)
      + square(r_b_mix[3]) * square(sd3)
    );
  }
